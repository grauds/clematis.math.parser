...
import org.clematis.math.reference.operations.SimpleFraction;
...
public class AlgorithmUtils
{
    ...
    public static String substituteParameters( iParameterProvider algorithm, Element element )
    {
        ...
        while( it.hasNext() )
        {
            ...
            {
                Element child = (Element)contentItem;
    ...
    public static void findStringVariants(iParameterProvider provider,
        ...
        throws Exception
    {
        if( exprRoot instanceof iMultivariantLogic )
        {
            iMultivariantLogic logic = (iMultivariantLogic)exprRoot;
     ...
     */
    public static String replaceParameters(String string, iParameterProvider provider, int currentLine, boolean skipInsideStrings)
        ...
        if ( provider != null && string != null && !string.trim().equals("") )
        {
                                             ...
                                             && !failedToFindParameter )
            {
                ...
                for (String token : tokens)
                {
                    ...
                    if (isParameterName(token) && ((skipInsideStrings && apos % 2 == 0) || !skipInsideStrings))
                    {
                        ...
                        if (param != null)
                        {
                            if (param.getContainer() != null)
                            {
                                ...
                                {
                                    throw new AlgorithmException("Undefined variable: " + param.getName() + " in line " + (currentLine + 1));
                                }
                            }
            ...
            {
                throw new AlgorithmException("Undefined variable: " + notFoundParams.toString().subSequence(0, notFoundParams.toString().length() - 1) + " in line " + (currentLine+1));
            }
        }
        return string;
    }
