...
import org.clematis.math.reference.SimpleValue;
...
//************************ CALCULATE PARAMETERS ************************
     ...
     * @throws org.clematis.math.reference.AlgorithmException on error.
     */
        ...
        for ( int i=0; i < lines.size(); i++ )
        {
            ...
            if ( ! key.isFunction() )
            {
                ...
                if ( param.isRandomized() )
                {
                    ...
                    if ( ac != null )
                    {
                        System.out.println("Parameter " + param.getName() + " is loaded from initial values set with " + ac.getValue(null));
                    }
                }
                if ( ac != null )
                {
...
//************************ FIND PARAMETERS' DEPENDENCIES *********************
    ...
    {
        if( exprItem instanceof iFunction )
        {
            iFunction function = (iFunction)exprItem;
        ...
        else if( exprItem instanceof StringConstant )
        {
            StringConstant constant = (StringConstant)exprItem;
        ...
        else if( exprItem instanceof ParameterReference )
        {
            ParameterReference paramRef = (ParameterReference)exprItem;
