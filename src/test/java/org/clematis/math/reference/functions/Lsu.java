...
import org.clematis.math.reference.MathUtils;
...
public class Lsu extends aFunction2
{
    ...
    private static int getLsuPower( String numberString, int sigDigits )
    {
        ...
        {
            exp = numberString.substring( scientificIndex + 1, numberString.length() );
        ...
        switch(decimalPointIndex)
        {
            ...
            default:
              ...
              /*      for(int i=decimalPointIndex + 1; i < numberString.length() ; i++)
                ...
                /*    if ( foundNonZero )
                    {*/
                        return - ( numberString.length() - (decimalPointIndex + 1) ) + (exp.equals("") ? 0 : Integer.parseInt(exp));
               /*     }
                    else
                    {
                        return (exp.equals("") ? 0 : Integer.parseInt(exp));
            ...
            case -1: //If the number is an integer
                {
                     ...
                     */
                   return numberString.length() - leastSignificantDigitIndex - 1 + (exp.equals("") ? 0 : Integer.parseInt(exp));
                }
            default:
                {
                    // find non zero digit
